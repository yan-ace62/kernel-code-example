...
static int databuflen = 2;

static ssize_t mychardev_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
    size_t maxdatalen = databuflen, ncopied;
    uint8_t databuf[32];
...
module_param(databuflen, int, 0444);
MODULE_PARM_DESC(databuflen, "write data buf len");

MODULE_LICENSE("GPL");
MODULE_AUTHOR("ace_yan<ace_yan62@126.com>");
